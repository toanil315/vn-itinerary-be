...
1. **Search Strategy**:
   - **Gemini Toggle**: Check `.github/.ck.json` (or `~/.github/.ck.json`) for `skills.research.useGemini` (default: `true`). If `false`, skip Gemini and use WebSearch.
   - **Gemini Model**: Read from `.github/.ck.json`: `gemini.model` (default: `gemini-3-flash-preview`)
