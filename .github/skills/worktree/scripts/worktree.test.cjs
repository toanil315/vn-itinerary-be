 ...
 * Run: node .github/skills/worktree/scripts/worktree.test.cjs
 */
...
test('invalid WORKTREE_ROOT env var fails safely', () => {
  ...
  try {
    execSync(
      `WORKTREE_ROOT="${invalidRoot}" node "${SCRIPT_PATH}" info --json`,
      {
        encoding: 'utf-8',
        cwd: STANDALONE_DIR,
        stdio: ['pipe', 'pipe', 'pipe'],
...
test('create handles only special characters', () => {
  ...
  const json = assertJSON(result.output);
  assert(
    json.error.code === 'INVALID_FEATURE_NAME',
    'Should report invalid feature name',
  );
});
