...
```
Skill(skill: "plann:red-team", args: "{plan-directory-path}")
```
...
| `auto`   | Use the `Skill` tool: `Skill(skill: "plann:validate", args: "{plan-directory-path}")` |
| `off`    | Skip validation                                                                       |

**Invocation (when prompt mode, user says yes):** Use the `Skill` tool to invoke `plan:validate`:

```
Skill(skill: "plann:validate", args: "{plan-directory-path}")
```
...
After creating: `node .github/scripts/set-active-plan.cjs {plan-dir}`
